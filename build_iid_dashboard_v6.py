"""
Builds iid_dashboard.html from iid_linewise, joined with order_report_oe
(for ord_time/hour only) and store_master (for site_name + cluster), in
kpnfresh_prod.duckdb.

Every row in iid_linewise is already an IID (discrepancy) line -- there is
no edited_qty<>0-style filter needed, unlike the Order Edit dashboard.
"Total orders" denominators (for IID %) are still sourced from
order_report_oe, same as the Order Edit dashboard, per business definition:
IID % = IID %'s logic, just with a different numerator table.

Tabs:
  - OVERALL: KPIs, Day/Week/Month trend, Top L1 / Top L2 / Top SKU
  - CLUSTER-WISE: pick a cluster -> KPIs, trend, Top L1/L2/SKU, Store-wise
    table, Hour-wise impact chart (hour comes from order_report_oe.ord_time,
    matched by order_number, since iid_linewise has no time column)

Usage:
    python build_iid_dashboard.py
Output:
    index.html (data embedded inside it -- this is the only file to upload)
"""
...
OUTPUT_DIR = r"C:\Users\KPN\Documents\IID"
os.makedirs(OUTPUT_DIR, exist_ok=True)
# Named index.html so GitHub Pages serves it automatically at the bare
# repo/folder URL, with no filename in the link.
OUTPUT_HTML = os.path.join(OUTPUT_DIR, "index.html")
IID_TABLE = "iid_linewise"
OE_TABLE = "order_report_oe"   # only used for ord_time (hour) lookups + the "total orders" denominators
STORE_TABLE = "store_master"

def main():
    con = duckdb.connect(DUCKDB_PATH, read_only=True)

    # Full row-level detail: every row in iid_linewise IS an IID line (no
    # extra filter needed, unlike OE's edited_qty<>0). site_name + cluster
    # come from store_master via outlet_id; hour comes from order_report_oe
    # via order_number (iid_linewise has no time column of its own).
    # Field mapping vs the OE dashboard's row shape (kept identical so the
    # rest of the JS below needs no changes):
    #   ksin        <- sku_code
    #   unit_price  <- refund_amount / quantity_number (derived; no unit
    #                  price column exists on iid_linewise)
    #   ordered_qty / received_qty <- quantity_number (iid_linewise has only
    #                  one qty column, so both are set the same)
    #   line_gmv / edited_gmv <- refund_amount (iid_linewise has only one
    #                  monetary column, so both are set the same)
    #   edited_qty  <- quantity_number
    rows = con.execute(f"""
        SELECT
            iid.dt::VARCHAR                                     AS dt,
            oe.ord_time                                          AS time_str,
            CAST(SPLIT_PART(oe.ord_time, ':', 1) AS INT)         AS hour,
            iid.order_number,
            iid.outlet_id,
            COALESCE(sm.site_name, iid.outlet_name, iid.outlet_id) AS site_name,
            COALESCE(sm.cluster_manager, 'UNASSIGNED')           AS cluster,
            iid.sku_code,
            iid.kpn_title                                        AS title,
            COALESCE(iid.merchandising_category_l1_name, 'UNKNOWN') AS l1,
            COALESCE(iid.merchandising_category_l2_name, 'UNKNOWN') AS l2,
            CASE WHEN iid.quantity_number <> 0
                 THEN iid.refund_amount / iid.quantity_number ELSE 0 END AS unit_price,
            iid.quantity_number                                  AS ordered_qty,
            iid.quantity_number                                  AS received_qty,
            iid.refund_amount                                    AS line_gmv,
            iid.quantity_number                                  AS edited_qty,
            iid.refund_amount                                    AS edited_gmv,
            iid.status,
            COALESCE(iid.return_reason, '')                     AS return_reason,
            COALESCE(iid.scan_type, '')                          AS scan_type
        FROM {IID_TABLE} iid
        LEFT JOIN (SELECT site_id, site_name, cluster_manager FROM {STORE_TABLE}) sm
            ON iid.outlet_id = sm.site_id
        LEFT JOIN (SELECT DISTINCT order_number, ord_time FROM {OE_TABLE}) oe
            ON iid.order_number = oe.order_number
        ORDER BY iid.dt, oe.ord_time
    """).fetchall()

    # Note: unlike order_report_oe, the LEFT JOIN to order_report_oe here can
    # leave time_str/hour NULL (order_number not found there), and iid_linewise
    # itself can have NULL ksin/title/status for some lines -- so every
    # dict-encoded (string) column below is coalesced to "" to avoid
    # None/str comparison errors when the dictionary values get sorted.
    rows_json = [
        {
            "dt": r[0] or "", "time": r[1] or "", "hour": r[2], "order_number": r[3] or "",
            "outlet_id": r[4] or "", "site_name": r[5] or "", "cluster": r[6] or "",
            "ksin": r[7] or "", "title": r[8] or "", "l1": r[9] or "", "l2": r[10] or "",
            "unit_price": float(r[11] or 0), "ordered_qty": r[12], "received_qty": r[13],
            "line_gmv": float(r[14] or 0), "edited_qty": r[15],
            "edited_gmv": float(r[16] or 0), "status": r[17] or "",
            "return_reason": r[18] or "",
            "scan_type": r[19] or "",
        }
        for r in rows
    ]
    ...
    _dict_cols = ["dt", "time", "order_number", "outlet_id", "site_name",
                  "cluster", "ksin", "title", "l1", "l2", "status", "return_reason", "scan_type"]
    _dicts = {}
    _idx = {}
    ...
    data_blob = {
        ...
        "generated_note": "DATA.rows = every row in iid_linewise (no status filter -- every line in this table is already an IID/discrepancy line). IID order (numerator) = distinct order_number present in iid_linewise. 'Total orders' denominators (day/store/L1/L2/SKU/grand_total) are sourced from order_report_oe, same logic as the Order Edit dashboard: that category's/store's own order count across ALL statuses, bucketed by date and summed for the selected date range -- i.e. IID % uses the identical denominator definition as IID %. Hour comes from order_report_oe.ord_time matched by order_number (iid_linewise has no time column). unit_price is derived as refund_amount/quantity_number (no unit price column exists on iid_linewise); ordered_qty/received_qty/edited_qty all map to quantity_number, and line_gmv/edited_gmv both map to refund_amount, since iid_linewise has only one qty column and one monetary column. Hour-wise Edited % = IID orders for that hour / total IID orders across ALL hours in the current window * 100 -- a SHARE of the window's IID orders, not a rate against all orders, so values across hours sum to ~100%.",
    }
    ...
    # Escape "</script" so an embedded string (e.g. a title or sku_code)
    # can't prematurely close the <script id="embedded-data"> tag it's
    # injected into. This is the standard safe way to embed arbitrary JSON
    ...
    print(f"IID Dashboard (data embedded) written to {OUTPUT_HTML}")
...
<title>KPN Fresh · IID Dashboard</title>
...
/* Distinct theme for the Scanned vs Not Scanned / L1-wise Scanned+IID block,
   so it reads visually apart from the rest of the (Order-Edit-style) chips. */
.scan-theme .blk-hdr{background:linear-gradient(115deg,#0f4c4c,#0a3a3a)}
.scan-theme tr.overall-row{background:#eef8f7 !important}
.iid-chip{display:inline-block;padding:3px 10px;border-radius:20px;font-weight:700;font-size:11px;
  background:rgba(13,148,136,.12);color:#0d9488;border:1px solid rgba(13,148,136,.22)}
.iid-chip.hi{background:rgba(126,34,206,.12);color:#7e22ce;border-color:rgba(126,34,206,.22)}
.iid-chip.mid{background:rgba(202,138,4,.12);color:#ca8a04;border-color:rgba(202,138,4,.22)}
...
<div id="loadOverlay">
  ...
  <div class="lo-title">KPN Fresh &middot; IID Dashboard</div>
...
<div class="topbar">
  <div>
    <div class="tb-title"><span class="ic">📊</span>IID Dashboard</div>
  ...
  <div class="main">
    ...
    <div class="block">
      <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <span id="trendTitle-overall">IID Impact</span>
...
<span class="custom-range-wrap" id="trendRange-overallCustomWrap" style="display:none"><input type="date" id="trendRange-overallFrom"><span class="crw-to">to</span><input type="date" id="trendRange-overallTo"></span>
          <select class="metric-sel" id="metricTrend-overall" style="width:150px">
            <option value="oe_pct" selected>IID %</option>
    ...
    <div class="block">
      <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <select class="metric-sel" id="drillFacet-overall" style="width:150px">
            <option value="reason" selected>Return Reason</option>
            <option value="l1">L1 Category</option>
          </select>
...
<span class="custom-range-wrap" id="drillRange-overallCustomWrap" style="display:none"><input type="date" id="drillRange-overallFrom"><span class="crw-to">to</span><input type="date" id="drillRange-overallTo"></span>
    ...
    <div class="two-col scan-theme">
      <div class="block">
        <div class="blk-hdr"><span class="crumb">Scanned vs Not Scanned</span></div>
        <div class="blk-body" style="max-width:320px;margin:0 auto"><canvas id="scanChart-overall" height="220"></canvas></div>
      </div>
      <div class="block">
        <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
          <span class="crumb">L1-wise Scanned % &amp; IID %</span>
          <select class="chart-range-sel" id="scanL1Range-overall" style="width:130px">
            <option value="custom">Custom</option>
            <option value="yesterday" selected>Yesterday</option>
            <option value="thisweek">This Week</option>
            <option value="thismonth">This Month</option>
...
<span class="custom-range-wrap" id="scanL1Range-overallCustomWrap" style="display:none"><input type="date" id="scanL1Range-overallFrom"><span class="crw-to">to</span><input type="date" id="scanL1Range-overallTo"></span>
        </div>
        <div class="tw"><table id="scanL1Table-overall"><thead></thead><tbody></tbody></table></div>
      </div>
    </div>
    <div class="two-col">
      <div class="block">
...
<span class="custom-range-wrap" id="storeRange-overallCustomWrap" style="display:none"><input type="date" id="storeRange-overallFrom"><span class="crw-to">to</span><input type="date" id="storeRange-overallTo"></span>
        ...
        <div class="tw"><table id="storeTable-overall"><thead><tr><th class="left">Store</th><th>IID %</th><th>Total Orders</th><th>IID Orders</th><th>Total IID Ksin distinct</th><th>IID Qty</th><th>Refund Amount</th></tr></thead><tbody></tbody></table></div>
      </div>
      <div class="block">
        <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
...
<div class="tabpane" id="pane-cluster">
  <div class="main">
    ...
    <div class="block">
      <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        <span id="trendTitle-cluster">IID Impact</span>
        <div style="display:flex;gap:8px;align-items:center">
...
<span class="custom-range-wrap" id="trendRange-clusterCustomWrap" style="display:none"><input type="date" id="trendRange-clusterFrom"><span class="crw-to">to</span><input type="date" id="trendRange-clusterTo"></span>
    ...
    <div class="block">
      <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <select class="metric-sel" id="drillFacet-cluster" style="width:150px">
            <option value="reason" selected>Return Reason</option>
            <option value="l1">L1 Category</option>
          </select>
...
<span class="custom-range-wrap" id="drillRange-clusterCustomWrap" style="display:none"><input type="date" id="drillRange-clusterFrom"><span class="crw-to">to</span><input type="date" id="drillRange-clusterTo"></span>
    ...
    <div class="two-col scan-theme">
      <div class="block">
        <div class="blk-hdr"><span class="crumb">Scanned vs Not Scanned</span></div>
        <div class="blk-body" style="max-width:320px;margin:0 auto"><canvas id="scanChart-cluster" height="220"></canvas></div>
      </div>
      <div class="block">
        <div class="blk-hdr" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px">
          <span class="crumb">L1-wise Scanned % &amp; IID %</span>
          <select class="chart-range-sel" id="scanL1Range-cluster" style="width:130px">
            <option value="custom">Custom</option>
            <option value="yesterday" selected>Yesterday</option>
            <option value="thisweek">This Week</option>
            <option value="thismonth">This Month</option>
...
<span class="custom-range-wrap" id="scanL1Range-clusterCustomWrap" style="display:none"><input type="date" id="scanL1Range-clusterFrom"><span class="crw-to">to</span><input type="date" id="scanL1Range-clusterTo"></span>
        </div>
        <div class="tw"><table id="scanL1Table-cluster"><thead></thead><tbody></tbody></table></div>
      </div>
    </div>
    <div class="two-col">
      <div class="block">
...
// iid_dashboard_data.json) into the same array-of-objects shape the
...
function unpackRows(){
  ...
  for(let i=0;i<n;i++){
    out[i] = {
      ...
      return_reason: d.return_reason[p.return_reason_i[i]],
      scan_type: d.scan_type[p.scan_type_i[i]],
    };
  }
  DATA.rows = out;
}
...
function gmvClass(v){ return 'neg'; /* Refund Amount is always shown in red, regardless of sign */ }
function oeChip(pct){
  if(pct == null) return '<td>n/a</td>';
  const cls = pct >= 15 ? 'hi' : (pct >= 7 ? 'mid' : '');
  return `<td><span class="oe-chip ${cls}">${pct.toFixed(2)}%</span></td>`;
}
function iidChip(pct){
  if(pct == null) return '<td>n/a</td>';
  const cls = pct >= 15 ? 'hi' : (pct >= 7 ? 'mid' : '');
  return `<td><span class="iid-chip ${cls}">${pct.toFixed(2)}%</span></td>`;
}
...
function aggregate(rows, keyFn){
  ...
  rows.forEach(r=>{
    const k = keyFn(r);
    if(!m.has(k)) m.set(k, {orders:new Set(), editedOrders:new Set(), ksins:new Set(), editedKsins:new Set(), qty:0, net_gmv:0, gross_gmv:0, scannedLines:0, notScannedLines:0, meta:{}});
    const e = m.get(k);
    e.orders.add(r.order_number);
    e.ksins.add(r.ksin);
    // Every row here is already an IID/discrepancy line (no >0 qty gate,
    // unlike the OE dashboard where edited_qty could legitimately be 0).
    ...
    // scan_type: EAN/WEIGHTIFY = Scanned, SKU_CODE/MANUAL = Not Scanned
    if(r.scan_type === 'EAN' || r.scan_type === 'WEIGHTIFY') e.scannedLines++;
    else e.notScannedLines++;
    if(!e.meta.title && r.title) e.meta.title = r.title;
    if(!e.meta.outlet_id && r.outlet_id) e.meta.outlet_id = r.outlet_id;
    if(!e.meta.return_reason && r.return_reason) e.meta.return_reason = r.return_reason;
  });
  return [...m.entries()].map(([k,v])=>({
    ...
    qty:v.qty, net_gmv:v.net_gmv, gross_gmv:v.gross_gmv,
    scanned_pct: (v.scannedLines + v.notScannedLines) ? (v.scannedLines / (v.scannedLines + v.notScannedLines)) * 100 : null,
    meta:v.meta
  }));
}
...
function scannedPct(rowsArr){
  let scanned = 0;
  rowsArr.forEach(r=>{ if(r.scan_type === 'EAN' || r.scan_type === 'WEIGHTIFY') scanned++; });
  return rowsArr.length ? (scanned / rowsArr.length) * 100 : null;
}

function renderOverallRow(labelColspan, label, editPct, qty, netGmv, extraCells, scannedPctVal){
  const pctCell = editPct == null ? '<td>n/a</td>' : `<td><span class="oe-chip">${editPct.toFixed(2)}%</span></td>`;
  const scanCell = scannedPctVal === undefined ? '' : `<td>${scannedPctVal == null ? 'n/a' : scannedPctVal.toFixed(1) + '%'}</td>`;
...
function renderKPIs(rows, targetId, oePct, totalOrdersDenom){
  ...
  const cards = [
    ...
    {lbl:'IID Orders (distinct)', val: fmtNum(editedOrders), ic:'\u2318'},
    {lbl:'IID % (IID \u00f7 Total Orders)', val: (oePct != null ? oePct.toFixed(2) + '%' : 'n/a'), ic:'\u25c9', oe:true},
    {lbl:'Refund Amount', val: fmtMoney(netGmv), cls: 'red', ic:'\u20b9'},
    {lbl:'Total IID Qty', val: fmtNum(totalQty), ic:'\u2261'},
    {lbl:'Total IID Ksin distinct', val: fmtNum(totalKsin), ic:'\u2317'},
  ];
...
const METRIC_LABELS = {
  net_gmv: 'Net IID GMV', qty: 'IID Qty', orders: 'IID Orders (distinct)', oe_pct: 'IID %'
};
...
function renderTrendChart(rows, canvasId, chartKey, metric, totalsByBucketMap, titleElId, titleSuffix, rangeSelId){
  ...
  if(titleElId){
    ...
    if(titleEl) titleEl.textContent = `IID Impact${rangeLabel ? ' (' + rangeLabel + ')' : ''}${titleSuffix || ''}`;
  }
  ...
  charts[chartKey] = new Chart(ctx, {
    ...
    options:{ responsive:true,
      ...
      scales:{ y:{title:{display:true, text: isPct ? 'IID %' : (isMoneyMetric(metric) ? 'GMV (\u20b9)' : label)} } },
...
// ---------- drill-down: Reason -> L1 -> L2 -> KSIN  (or L1 -> L2 -> KSIN directly, per facet dropdown) ----------
const drillState = { overall: {facet:'reason', level:'reason', l1:null, l2:null, reason:null}, cluster: {facet:'reason', level:'reason', l1:null, l2:null, reason:null} };
const lastDrillRows = { overall: [], cluster: [] };

function drillInto(suffix, key){
  const st = drillState[suffix];
  if(st.level === 'reason'){ st.reason = key; st.level = 'l1'; }
...
function drillReset(suffix){
  const facet = drillState[suffix].facet;
  drillState[suffix] = {facet, level: facet === 'reason' ? 'reason' : 'l1', l1:null, l2:null, reason:null};
  renderDrillTable(lastDrillRows[suffix], suffix);
}
function drillSetFacet(suffix, facet){
  drillState[suffix] = {facet, level: facet === 'reason' ? 'reason' : 'l1', l1:null, l2:null, reason:null};
  renderDrillTable(lastDrillRows[suffix], suffix);
}
...
function drillBackToReason(suffix){
  const st = drillState[suffix];
  st.level = 'l1'; st.l1 = null; st.l2 = null; // keep st.reason
  renderDrillTable(lastDrillRows[suffix], suffix);
}
window.drillReset = drillReset; window.drillUpToL2 = drillUpToL2; window.drillBackToReason = drillBackToReason; window.drillSetFacet = drillSetFacet;
...
function setupDrillClickHandler(suffix){
  ...
  document.getElementById(`drillFacet-${suffix}`).addEventListener('change', (e)=>{
    drillSetFacet(suffix, e.target.value);
  });
}
...
function renderDrillTable(rows, suffix){
  ...
  // If a return_reason has been drilled into, scope the L1/L2/KSIN levels
  // below to just that reason's rows. At the 'reason' level itself (top),
  // st.reason is still null so scopedRows === rows (all reasons visible).
  const scopedRows = st.reason ? rows.filter(r=>r.return_reason === st.reason) : rows;
  ...
  let overallScannedPct = null;

  if(st.level === 'reason'){
    // IID % here = that reason's IID orders / overall IID orders in the
    // current window (a SHARE of total IID orders) -- there is no
    // return_reason breakdown in order_report_oe to divide by instead.
    const overallIIDOrders = new Set(scopedRows.map(r=>r.order_number)).size;
    agg = aggregate(scopedRows, r=>r.return_reason || 'UNSPECIFIED');
    agg.forEach(o=>{
      o.totalOrders = overallIIDOrders;
      o.editPct = overallIIDOrders ? (o.edited_orders / overallIIDOrders) * 100 : null;
    });
    cols = ['Return Reason','IID %','Overall IID Orders','IID Orders','Total IID Ksin distinct','IID Qty','Scanned %','Refund Amount'];
    keyLabelFn = o=>o.key;
    clickable = true;
    overallQty = scopedRows.reduce((a,r)=>a+Math.abs(r.edited_qty),0);
    overallGmv = scopedRows.reduce((a,r)=>a+r.edited_gmv,0);
    overallKsinCount = new Set(scopedRows.map(r=>r.ksin)).size;
    overallEditedOrders = overallIIDOrders;
    overallTotalOrders = totalOrdersAllStatus; // whole store's total orders (true rate, same basis as the L1 view's Overall row)
    overallScannedPct = scannedPct(scopedRows);

  } else if(st.level === 'l1'){
    const totalEditedOrdersAll = new Set(scopedRows.map(r=>r.order_number)).size;
    agg = aggregate(scopedRows, r=>r.l1);
    agg.forEach(o=>{
    ...
    cols = ['L1','IID %','Total Orders','IID Orders','Total IID Ksin distinct','IID Qty','Scanned %','Refund Amount'];
    keyLabelFn = o=>o.key;
    clickable = true;
    overallQty = scopedRows.reduce((a,r)=>a+Math.abs(r.edited_qty),0);
    overallGmv = scopedRows.reduce((a,r)=>a+r.edited_gmv,0);
    ...
    const subset = scopedRows.filter(r=>r.l1 === st.l1);
    agg = aggregate(subset, r=>r.l2);
    const totalEditedForL1 = new Set(subset.map(r=>r.order_number)).size;
    agg.forEach(o=>{
    ...
    cols = ['L2','IID %','Total Orders','IID Orders','Total IID Ksin distinct','IID Qty','Scanned %','Refund Amount'];
    keyLabelFn = o=>o.key;
    clickable = true;
    ...
    overallScannedPct = scannedPct(subset);

  } else {
    const subset = scopedRows.filter(r=>r.l1 === st.l1 && r.l2 === st.l2);
    agg = aggregate(subset, r=>r.ksin);
    const totalEditedForL2 = new Set(subset.map(r=>r.order_number)).size;
    agg.forEach(o=>{
    ...
    cols = ['SKU','Title','Return Reason','IID %','Total Orders','IID Orders','IID Qty','Scanned %','Refund Amount'];
    keyLabelFn = o=>o.key;
    includeTitle = true;
    clickable = false;
    overallQty = subset.reduce((a,r)=>a+Math.abs(r.edited_qty),0);
  ...
  thead.innerHTML = '<tr>' + cols.map(c=>`<th class="${c==='L1'||c==='L2'||c==='SKU'||c==='Title'||c==='Return Reason'?'left':''}">${c}</th>`).join('') + '</tr>';
  ...
  const overallRowHtml = renderOverallRow(includeTitle ? 3 : 1, 'Overall', overallPct, overallQty, overallGmv, overallExtra, overallScannedPct);
  ...
  tbody.innerHTML = overallRowHtml + agg.map(o=>{
    ...
    return `<tr${rowAttrs}>
      ...
      ${includeTitle ? `<td class="left">${o.meta.title||''}</td><td class="left">${o.meta.return_reason||''}</td>` : ''}
      ${oeChip(o.editPct)}
      <td>${fmtNum(o.totalOrders)}</td>
      <td>${fmtNum(o.edited_orders)}</td>
      ${includeTitle ? '' : `<td>${fmtNum(o.edited_ksin_count)}</td>`}
      <td>${fmtNum(o.qty)}</td>
      <td>${o.scanned_pct == null ? 'n/a' : o.scanned_pct.toFixed(1) + '%'}</td>
      <td class="${gmvClass(o.net_gmv)}">${fmtMoney(o.net_gmv)}</td>
    </tr>`;
  }).join('');

  const crumb = document.getElementById(`drillCrumb-${suffix}`);
  let html = `<span class="cr-link" onclick="drillReset('${suffix}')">More Analysis: ${st.facet === 'reason' ? 'Return Reason / L1 / L2 / KSIN-wise' : 'L1 / L2 / KSIN-wise'}</span>`;
  if(st.reason){
    const reasonSeg = st.level === 'l1'
      ? `<span class="cr-current">${st.reason}</span>`
      : `<span class="cr-link" onclick="drillBackToReason('${suffix}')">${st.reason}</span>`;
    html += `<span class="cr-sep">\u203a</span>${reasonSeg}`;
  }
  if(st.level === 'l2'){
...
function renderStoreDrillTable(rows, suffix){
  ...
  if(st.level === 'store'){
    ...
    const totalEditedAll = new Set(rows.map(r=>r.order_number)).size;
    agg.forEach(o=>{
      o.totalOrders = sumStoreTotalsForRange(o.key, rangeVal);
    ...
    cols = ['Store','Outlet ID','IID %','Total Orders','IID Orders','Total IID Ksin distinct','IID Qty','Refund Amount'];
    includeOutlet = true;
    ...
    const totalEditedForStore = new Set(subset.map(r=>r.order_number)).size;
    ...
    cols = ['L1','IID %','Total Orders','IID Orders','Total IID Ksin distinct','IID Qty','Refund Amount'];
    ...
    cols = ['L2','IID %','Total Orders','IID Orders','Total IID Ksin distinct','IID Qty','Refund Amount'];
    overallEditedOrders = totalEditedForL1;
    overallEditPct = scopeTotalOrders ? (overallEditedOrders / scopeTotalOrders) * 100 : null;
    overallQty = subset.reduce((a,r)=>a+Math.abs(r.edited_qty),0);
    overallGmv = subset.reduce((a,r)=>a+r.edited_gmv,0);
    ...
    cols = ['SKU','Title','IID %','Total Orders','IID Orders','IID Qty','Refund Amount'];
    overallEditedOrders = totalEditedForL2;
    overallEditPct = scopeTotalOrders ? (overallEditedOrders / scopeTotalOrders) * 100 : null;
    overallQty = subset.reduce((a,r)=>a+Math.abs(r.edited_qty),0);
    overallGmv = subset.reduce((a,r)=>a+r.edited_gmv,0);
  ...
  thead.innerHTML = '<tr>' + cols.map(c=>`<th class="${c==='Store'||c==='Outlet ID'||c==='L1'||c==='L2'||c==='SKU'||c==='Title'?'left':''}">${c}</th>`).join('') + '</tr>';
...
// hour totals.
// ================================================================
function renderScanL1Table(rows, suffix){
  const totalEditedOrdersAll = new Set(rows.map(r=>r.order_number)).size;
  const agg = aggregate(rows, r=>r.l1);
  agg.forEach(o=>{
    // Same IID % logic as the L1 view in More Analysis: this L1's share of
    // ALL edited orders in the current window.
    o.editPct = totalEditedOrdersAll ? (o.edited_orders / totalEditedOrdersAll) * 100 : null;
  });
  agg.sort((a,b)=>(b.editPct||0)-(a.editPct||0));

  const thead = document.querySelector(`#scanL1Table-${suffix} thead`);
  thead.innerHTML = '<tr><th class="left">L1</th><th>Scanned %</th><th>IID %</th></tr>';

  const tbody = document.querySelector(`#scanL1Table-${suffix} tbody`);
  tbody.innerHTML = agg.map(o=>`<tr>
    <td class="left">${o.key}</td>
    <td>${o.scanned_pct == null ? 'n/a' : o.scanned_pct.toFixed(1) + '%'}</td>
    ${iidChip(o.editPct)}
  </tr>`).join('');
...
function renderScanChart(rows, canvasId, chartKey){
  // EAN + WEIGHTIFY = Scanned; SKU_CODE + MANUAL = Not Scanned (line-count basis).
  let scanned = 0, notScanned = 0;
  rows.forEach(r=>{
    if(r.scan_type === 'EAN' || r.scan_type === 'WEIGHTIFY') scanned++;
    else notScanned++;
  });
  ...
  charts[chartKey] = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: [`Scanned (${pct(scanned)}%)`, `Not Scanned (${pct(notScanned)}%)`],
      datasets: [{
        data: [scanned, notScanned],
        backgroundColor: ['#0d9488', '#7e22ce'],
        borderWidth: 0
      }]
    },
    options: {
    ...
    options: { 
      ...
      scales: { 
        y: { 
          ...
          title: { display: true, text: isPct ? 'IID %' : label },
...
function renderOverall(){
  ...
  const scanL1RangeVal = getEffectiveRange('scanL1Range-overall');
  const scanL1Rows = rowsInWindow(DATA.rows, scanL1RangeVal, null, null);
  try { renderScanChart(scanL1Rows, 'scanChart-overall', 'overall_scan'); } catch(err){ console.error('Scan chart failed:', err); }
  try { renderScanL1Table(scanL1Rows, 'overall'); } catch(err){ console.error('Scan L1 table failed:', err); }
...
function renderCluster(){
  ...
  const scanL1RangeVal = getEffectiveRange('scanL1Range-cluster');
  const scanL1Rows = rowsInWindow(clusterBaseRows, scanL1RangeVal, null, null);
  try { renderScanChart(scanL1Rows, 'scanChart-cluster', 'cluster_scan'); } catch(err){ console.error('Scan chart failed:', err); }
  try { renderScanL1Table(scanL1Rows, 'cluster'); } catch(err){ console.error('Scan L1 table failed:', err); }
...
const CHART_RANGE_SEL_IDS = [
  'drillRange-overall', 'storeRange-overall', 'hourRange-overall', 'scanL1Range-overall',
  'drillRange-cluster', 'storeRange-cluster', 'hourRange-cluster', 'scanL1Range-cluster'
];
...
//           IID Impact (trend-range-sel) defaults to "Last 30 days".
//  - Month: top KPIs + every chart-range-sel default to "This Month";
//           IID Impact defaults to "All dates" (full min ord_dt -> max ord_dt).
